...
export async function GET(req: NextRequest) {
  ...
  const facturas = await getFacturasRango(empresaId, new Date("2000-01-01"), new Date(hasta))
  ...
  const buffer = await renderPDF(React.createElement(BalanceDocument, {
    empresa,
  ...
  return new NextResponse(buffer, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="balance-${hasta}.pdf"`,
    },
  })
}
